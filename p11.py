...
for i in range(0, rowCount):
    for n in range(0,columnCount):
        # check straight down
        if i <= rowCount-4:
            product = grid[i][n] * grid[i+1][n] * grid[i+2][n] * grid[i+3][n]
            products.append(product)
        # check right
        if n <= columnCount-4:
            product = grid[i][n] * grid[i][n+1] * grid[i][n+2] * grid[i][n+3]
            products.append(product)
        # check diagonal down right
        if i < rowCount-4 and n <= columnCount-4:
            product = grid[i][n] * grid[i+1][n+1] * grid[i+2][n+2] * grid[i+3][n+3]
            products.append(product)
        # check diagonal down left
        if i < rowCount-4 and n >= 3:
            product = grid[i][n] * grid[i+1][n-1] * grid[i+2][n-2] * grid[i+3][n-3]
            products.append(product)
